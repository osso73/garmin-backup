...
This is the first working version, but not really final. I need to add some messages to inform the user, and then I will add the testing. This version also allows to use a FakeGarmin class for test purposes, so it does all the steps without really making any connection to GarminConnect, just using some activity data I've downloaded.
